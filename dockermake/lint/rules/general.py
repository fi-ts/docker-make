    ...
    def rule0_1_5(self):
        ...
        for label in instructions:
            if not label.contains("maintainer", False):
                self.error(self.rule0_1_5, line_number=label.physical_line_number)
            ...
            if maintainers:
                for maintainer in maintainers.split(","):
                    if "@" not in maintainer:
                        self.error(self.rule0_1_6, line_number=label.physical_line_number)
    ...
    def rule3(self):
        ...
        for instruction in instructions:
            if instruction.username is None:
                self.error(self.rule3, line_number=instruction.physical_line_number)
    ...
    def rule4(self):
        ...
        for instruction in instructions:
            if instruction.image and len(instruction.image) < 2:
                self.error(self.rule4, substitution=instruction.image, line_number=instruction.physical_line_number)
    ...
    def rule5(self):
        ...
        for instruction in instructions:
            if instruction.tag is None:
                self.error(self.rule5, line_number=instruction.physical_line_number)

    def rule6(self):
        """ADD is prohibited"""
        instructions = self.dockerfile.get_instructions_of_type(Keywords.ADD)
        if instructions:
            self.error(self.rule6)
    ...
    def rule9(self):
        ...
        for instruction in instructions:
            if instruction.argument.strip().startswith("sudo"):
                self.error(self.rule9, line_number=instruction.physical_line_number)
    ...
    def rule10(self):
        ...
        for physical_line in physical_lines:
            ...
            if len(physical_line) > 100:
                self.warning(self.rule10, line_number=line_number)
    ...
    def rule11_0(self):
        ...
        for physical_line in physical_lines:
            ...
            if and_index > 0 or or_index > 0:
                self.warning(self.rule11_0, line_number=line_number)
    ...
    def rule11_1(self):
        ...
        for physical_line in physical_lines:
            ...
            if abs(and_index) != 1 or abs(or_index) != 1:
                self.warning(self.rule11_1, line_number=line_number)
    ...
    def rule12_1(self):
        ...
        for cmd in instructions:
            if not ExecFormBase.is_pure_double_quoted_exec_form(cmd):
                self.error(self.rule12_1, line_number=cmd.physical_line_number)
    ...
    def rule12_2(self):
        ...
        for entrypoint in instructions:
            if not ExecFormBase.is_pure_double_quoted_exec_form(entrypoint):
                self.error(self.rule12_2, line_number=entrypoint.physical_line_number)
    ...
    def rule12_3(self):
        ...
        for volume in instructions:
            if not ExecFormBase.is_pure_double_quoted_exec_form(volume):
                self.error(self.rule12_3, line_number=volume.physical_line_number)
        ...
        for instruction in instructions:
            ...
            if maintainers:
                for maintainer in maintainers.split(","):
                    ...
                    if not name:
                        self.error(self.rule13_0, line_number=instruction.physical_line_number)
                    if not email:
                        self.error(self.rule13_0, line_number=instruction.physical_line_number)
        ...
        for maintainer_instruction in instructions:
            if maintainer_instruction.maintainers:
                for maintainer in maintainer_instruction.maintainers:
                    name, email = parseaddr(maintainer)
                    if not name:
                        self.error(self.rule13_1, line_number=maintainer_instruction.physical_line_number)
                    if not email:
                        self.error(self.rule13_1, line_number=maintainer_instruction.physical_line_number)
